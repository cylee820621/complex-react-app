    ...
    if (state.requestCount) {
      ...
      async function fetchResults() {
        try {
          ...
          console.log("there is a problem in cancel");
        }
      }
      fetchResults();
    ...
    <div className="search-overlay">
      <div className="search-overlay-top shadow-sm">
        <div className="container container--narrow">
          ...
          <span onClick={() => appDisptach({ type: "closeSearch" })} className="close-live-search">
      ...
      <div className="search-overlay-bottom">
        <div className="container container--narrow py-3">
          ...
          <div className={"live-search-results " + (state.show == "results" ? "live-search-results--visible" : "")}>
            {Boolean(state.results.length) && (
              <div className="list-group shadow-sm">
                ...
                {state.results.map((post) => {
                  ...
                  return (
                    <Link onClick={() => appDisptach({ type: "closeSearch" })} key={post._id} to={`/post/${post._id}`} className="list-group-item list-group-item-action">
            ...
            {!Boolean(state.results.length) && <p className="alert alert-danger text-center shadow-sm">Sorry, no result</p>}
          </div>
        </div>
      </div>
    </div>
